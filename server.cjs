...
${data.documents.map((d, i) => `DOC ${i + 1}:
${d.content}`).join("\n\n")}
...
TASK: Conduct a rigorous audit of the draft proposal below to find robotic styling, overly uniform syntax patterns, clich\xE9s, artificial transitional phrases, and make strategic adjustments to ensure it reads with genuine human flair, emotion, and pass-grade authenticity.
